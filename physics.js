...
const Physics = (() => {
  ...
  function createBikeBody(pos) {
    // Capsule-ish: compound of 3 boxes
    const body = new CANNON.Body({ mass: 220, material: world._bikeMat });
    // Main chassis
    body.addShape(new CANNON.Box(new CANNON.Vec3(0.35, 0.3, 0.9)));
    // Front
    body.addShape(new CANNON.Box(new CANNON.Vec3(0.15, 0.2, 0.4)),
      new CANNON.Vec3(0, -0.05, 0.85));
    // Rear
    body.addShape(new CANNON.Box(new CANNON.Vec3(0.15, 0.2, 0.35)),
      new CANNON.Vec3(0, -0.05, -0.85));

    body.position.set(pos.x, pos.y, pos.z);
    body.linearDamping  = 0.4;
    body.angularDamping = 0.98;   // prevent tumbling
    body.allowSleep = false;

    // Prevent rotation on X and Z (no flipping)
    body.fixedRotation = false;
    // But heavily damp angular
    body.angularFactor.set(0.02, 1, 0.02);
