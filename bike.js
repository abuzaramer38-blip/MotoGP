...
const Bike = (() => {
  ...
  const CFG = {
    ...
    groundY:       1.0,   // spawn/ride height — raised so bike never clips ground
    nitroMult:     1.6,
  ...
  function update(dt, input) {
    ...
    // LEFT steer → positive yaw (turn left), RIGHT steer → negative yaw (turn right)
    const yawRate = -state.steer * steerRate * speedFactor;
    body.angularVelocity.y = Utils.lerp(body.angularVelocity.y, yawRate, 10 * dt);

    // Extract the bike's local forward vector from its quaternion.
    // In Three.js/Cannon the bike faces -Z by default.
    // forward = rotate (0,0,-1) by body quaternion
    const q  = body.quaternion;
    // Rotate (0,0,-1): standard quaternion sandwich gives:
    const fx = 2*(q.x*q.z + q.w*q.y);          // X component of forward
    const fz = q.w*q.w - q.x*q.x - q.y*q.y + q.z*q.z;  // Z component (negated for -Z face)
    // fz above gives the Z of the rotated +Z axis; we want -Z so negate:
    body.velocity.x =  fx * state.speed;
    body.velocity.z = -fz * state.speed;        // negative = forward into the track
    body.velocity.y = Utils.clamp(body.velocity.y, -20, 5);

    // Pin the bike to ground height — prevents physics from sinking or flying
  ...
  return { init, reset, update, updateCamera, upgradeEngine, upgradeTires, addNitro, getState, getMesh, getCFG };
})();
